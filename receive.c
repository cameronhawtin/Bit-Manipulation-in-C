...

{
	...
	parity = 0; // base case parity is 0
	storedParity = isShortBitSet(*num, 1); // now we check the parity of the initially stored parity
	...
	if (storedParity != parity) // if the parities are not the same, we have an error
		bitNumber += 1; // in this case add 2^0 to bitNumber
	

	// we do the same for the remaining parity bits:
    // P2
	
	parity = 0; // base case parity is 0
	storedParity = isShortBitSet(*num, 2); // now we check the parity of the initially stored parity
	...
	if (storedParity != parity) // if the parities are not the same, we have an error
		bitNumber += 2; // in this case add 2^1 to bitNumber

    // P4
	parity = 0; // base case parity is 0
	storedParity = isShortBitSet(*num, 4); // now we check the parity of the initially stored parity
	...
	if (storedParity != parity) // if the parities are not the same, we have an error
		bitNumber += 4; // in this case add 2^2 to bitNumber	
	
    // P8
	parity = 0; // base case parity is 0
	storedParity = isShortBitSet(*num, 8); // now we check the parity of the initially stored parity
	...
	if (storedParity != parity) // if the parities are not the same, we have an error
		bitNumber += 8; // in this case add 2^3 to bitNumber
